...
int64_t TrackEditor::CalcInitialDelay()
{
	...
	int64_t diff = m_sampleTimes[i].dts - m_sampleTimes[i].cts;
...
void TrackEditor::UpdateStts()
{
    ...
    for (size_t i = 0; i < GetFrameCount(); ++i) {
	int32_t delta =
	    static_cast<int32_t>(m_sampleTimes[i+1].dts - m_sampleTimes[i].dts);
	if (delta != prev_delta) {
	    ++sttsIndex;
